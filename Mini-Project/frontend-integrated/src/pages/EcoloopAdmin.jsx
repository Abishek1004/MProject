      ...
      <main className="flex-1 flex flex-col min-w-0 transition-all duration-500">
        ...
        <div className="p-10 space-y-10 overflow-x-hidden">
                  ...
                  <div className="overflow-x-auto -mx-2">
                    <table className="w-full text-left min-w-[500px]">
                      ...
                      <tbody className="text-sm font-bold divide-y divide-slate-100 dark:divide-slate-800">
                        {users.slice(0, 4).map((order, i) => (
                          <tr key={i} className="group transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-900/30">
                            <td className="px-3 py-6">
                              <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-slate-900 flex items-center justify-center text-lg text-emerald-500">
                                  <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 2v20"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
                                </div>
                                <span className="text-slate-600 dark:text-slate-300">Water Bottle</span>
                              </div>
                            </td>
                            <td className="px-3 py-6">
                              <span className="text-emerald-500">{order.name}</span>
                            </td>
                            <td className="px-3 py-6 text-slate-400">#8441573</td>
                            <td className="px-3 py-6 text-slate-400 font-medium">{order.date || '27 Jun 2025'}</td>
                            <td className="px-3 py-6">
          ...
          {activeTab === 'Manage Users' && (
             <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="bg-white dark:bg-slate-800 rounded-[48px] shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden">
                <div className="p-10 border-b border-slate-100 dark:border-slate-800 flex flex-wrap justify-between items-center gap-6">
                   <div>
                      <h3 className="text-3xl font-black tracking-tight">User Operations</h3>
                      <p className="text-slate-400 font-bold text-sm mt-1 uppercase tracking-widest">Perform Admin Actions</p>
                   </div>
                   <div className="flex gap-4">
                      <button className="bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest active:scale-95 transition-all">
                        Export CSV
                      </button>
                      <button 
                        onClick={handleCreateUser}
                        className="bg-emerald-500 text-white px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-[0.15em] shadow-lg shadow-emerald-500/20 active:scale-95 transition-all"
                      >
                        + Create Account
                      </button>
                   </div>
                ...
                {loading ? (
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      <thead>
                        <tr className="bg-slate-50/50 dark:bg-slate-900/50 text-slate-400 text-[11px] font-black uppercase tracking-[0.2em]">
                          ...
                          <th className="px-10 py-6 text-right">Perform Action</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {filteredUsers.map((user, i) => (
                          <tr key={user.id || i} className="hover:bg-slate-50/30 dark:hover:bg-slate-900/20 transition-all duration-300">
                            <td className="px-10 py-8">
                              <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-[18px] bg-slate-900 text-white flex items-center justify-center font-black shadow-lg shadow-slate-900/20">
                            ...
                            <td className="px-10 py-8">
                              <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest bg-slate-100 dark:bg-slate-700 px-3 py-1 rounded-lg">
                                {user.role || 'USER'}
                              </span>
                            </td>
                            <td className="px-10 py-8">
                              <span className={`text-[10px] font-black uppercase px-4 py-1.5 rounded-full bg-emerald-500 text-white`}>
                                ACTIVE
                              </span>
                            </td>
                            <td className="px-10 py-8 text-xs font-black text-slate-400">{user.date || 'N/A'}</td>
                            <td className="px-10 py-8 text-right">
                                <div className="flex justify-end gap-2">
                                  <button className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-700 text-slate-500 hover:text-emerald-500 transition-all hover:shadow-sm">
                                  ...
                                  <button 
                                    ...
                                    className="p-2.5 rounded-xl bg-slate-100 dark:bg-slate-700 text-slate-500 hover:text-red-500 transition-all hover:shadow-sm"
                                  >
