...
function ModelCard({ model, catColor, onClick }) {
  ...
  return (
    <motion.button
      onClick={onClick}
      className="w-full bg-white rounded-2xl p-5 text-left cursor-pointer border-none"
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <h3 className="font-poppins font-bold text-[1.05rem] text-slate-800 pr-2">
          {model.name}
        </h3>
        <motion.span
...
export default function ModelsPage({ nav, go, goBack, canGoBack }) {
  ...
  return (
    <div>
      <motion.div
        className="px-5 py-10 bg-white border-b border-slate-100"
